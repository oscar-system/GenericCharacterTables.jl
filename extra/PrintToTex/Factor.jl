...
roots = Dict{RootKey,Root}([
  ...
  (
    RootKey(QQFieldElem[-1, 0, 0, 0, 1], QQFieldElem[0, 1, 0, -1, 0], Symbol("sqrt(2)")),
    GEWC8ppZ2,
  ),
  (
    RootKey(QQFieldElem[-1, 0, 0, 0, 1], QQFieldElem[0, -1, 0, 1, 0], Symbol("sqrt(2)")),
  ...
  (
    RootKey(QQFieldElem[1, 0, 1, 0, 1], QQFieldElem[0, -1, 0, -1, 0], Symbol("sqrt(2)")),
    GEWC24pp,
  ),
...
function Base.show(io::IO, z::Zeta)
  if isempty(z.zetas)
    ...
    if z.rootformat
      mainpart = join(
        map(pair -> "$(root(pair[1]).str)^($(pair[2]))", collect(z.zetas)), " * "
      )
...
function Base.show(io::IO, m::MIME{Symbol("text/latex")}, z::Zeta)
  if isempty(z.zetas)
    ...
    if z.rootformat
      mainpart = join(
        map(
          pair -> "{$(root(pair[1]).tex)}^{\\left($(repr("text/latex",pair[2]))\\right)}",
          collect(z.zetas),
        ),
        " ",
      )
      ...
      mainpart = join(
        map(
          pair -> "\\zeta_{$(repr("text/latex",pair[1]))}^{$(repr("text/latex",pair[2]))}",
          collect(z.zetas),
        ),
      )
      coeffs = collect(coefficients(z.modulus))
...
Base.show(io::IO, m::MIME{Symbol("text/latex")}, z::ZetaSum) =
  print(io, join(repr.("text/latex", z.summands), " + "))
...
Base.show(io::IO, m::MIME{Symbol("text/latex")}, z::ZetaFrac) = print(
  io, "\\frac{$(repr("text/latex", z.numerator))}{$(repr("text/latex",z.denominator))}"
)
...
"""
function zeta(
  a::GenericCharacterTables.Cyclo{T}, rootformat::Bool=false
...
function zeta(
  a::GenericCharacterTables.CycloSum{T}, rootformat::Bool=false
) where {T<:PolyRingElem}
  return ZetaSum{T}(zeta.(a.summands, Ref(rootformat)))
end
function zeta(
  a::GenericCharacterTables.CycloFrac{T}, rootformat::Bool=false
) where {T<:PolyRingElem}
