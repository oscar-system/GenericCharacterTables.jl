...
classlength = R.([
	...
	(q-1)*(q^2+q+1)*q^3,
	(q+1)*(q-1)^2*q^3])
classtypeorder = R.([q-1, q-1, q-1, q^2-3*q+2, q^2-3*q+2, 1//6*q^3-q^2+11//6*q-1, 1//2*q^3-q^2+1//2*q, 1//3*q^3-1//3*q])
charinfo = Vector{Any}[[[1,1,1]], [[2,1]], [[3]]]
chardegree = R.([q^3, (q+1)*q, 1])

information = "- Information about the tables of unipotent characters for GL_3(q).
