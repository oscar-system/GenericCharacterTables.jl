...
const UPolyRing = parent_type(UPoly)
...
function normal_form(f::AbstractAlgebra.Generic.UnivPoly{ZZRingElem, AbstractAlgebra.Generic.MPoly{ZZRingElem}}, m::Int64)
	...
	for k in reverse(0:(K_m-1))
		...
		for (p,scheme) in schemes
			...
			elseif factor <= length(scheme)
				exponent=factorization[p]-scheme[factor]
				if exponent > 0
...
function strip_zeros!(f::Dict{UPolyFrac, UPoly})
	for (key,value) in f
		if iszero(value)
			delete!(f, key)
		end
...
const GenericCycloID = AbstractAlgebra.CacheDictType{Ring, GenericCycloRing}()
...
isone(x::GenericCyclo) = isone(length(x.f)) && isone(get(x.f, base_ring(parent(x))(0)//base_ring(parent(x))(1), 0))  # TODO

is_unit(f::GenericCyclo) = false  # TODO

characteristic(R::GenericCycloRing) = characteristic(base_ring(R))

# Canonical unit

canonical_unit(x::GenericCyclo) = canonical_unit(x.f[keys(x.f)[1]])  # TODO
...
function show(io::IO, x::GenericCyclo)  # TODO
	if iszero(x)
		print(io, "0")
		return
	end
...
function +(x::GenericCyclo, y::GenericCyclo)
	parent(x) != parent(y) && error("Incompatible rings")
	f=mergewith(+, x.f, y.f)
	strip_zeros!(f)
	return parent(x)(f)
end
...
function divexact(f::GenericCyclo, g::GenericCyclo; check::Bool = true)  # TODO
...
function inv(f::GenericCyclo)  # TODO
	R = parent(f)
...
function mul!(x::GenericCyclo, y::GenericCyclo, z::GenericCyclo)  # TODO
	x.f = (y*z).f
	return x
end

function add!(x::GenericCyclo, y::GenericCyclo, z::GenericCyclo)  # TODO
	x.f = (y+z).f
	return x
end
...
function (R::GenericCycloRing)(f::Dict{UPolyFrac, UPoly}; simplify::Bool=true)  # TODO check parent rings?
	...
	return GenericCyclo(filter(((g,c),) -> !iszero(c), fp), R)
end

function (R::GenericCycloRing)(x::GenericCyclo)
	R != parent(x) && error("Unable to coerce element")
	return x
end

# Parent constructor

function generic_cyclotomic_ring(R::UPolyRing, congruence::Union{Tuple{QQFieldElem, QQFieldElem}, Nothing}=nothing, cached::Bool=true)  # TODO
...
function Base.conj(x::GenericCyclo)
	f = Dict{UPolyFrac, UPoly}()
	for (key,value) in x.f
		f[-key]=value
	end
